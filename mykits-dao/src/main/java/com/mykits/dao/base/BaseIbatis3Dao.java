...
public abstract class BaseIbatis3Dao<E, PK extends Serializable> extends DaoSupport implements EntityDao<E, PK> {
	...
	public String getFindByExampleKeyStatement() {
		return getIbatisMapperNamesapce() + ".findByExample";
	}
	...
	public List findByExample(BaseIbatis3Example ex) {
		return sqlSessionTemplate.selectList(getFindByExampleKeyStatement(), ex);
	}

	public E getByExample(BaseIbatis3Example ex) {
		List list = findByExample(ex);
		return list != null && list.size() == 1 ? (E) list.get(0) : null;
	}
